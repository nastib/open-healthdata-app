...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!await dataSourcePermissions.canView(user.profile, validationId.data)) {
      throw createError({
        statusCode: 403,
        statusMessage: 'Forbidden - Insufficient permissions to view this data source'
      })
    }

    const { data, error } = await dataSourceServices.fetchSourceById(validationId.data)

    if (error || !data) {
      const serviceError = error as ErrorWithStatus
      throw createError({
        statusCode: serviceError?.statusCode || 404,
        statusMessage: serviceError?.statusMessage || `Data source with ID ${validationId.data} not found`,
        data: serviceError
      })
    }
