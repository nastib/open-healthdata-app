...
const { deleteVariable } = new VariableServices()
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!await variablePermissions.canDelete(user.profile, validationId.data)) {
      throw createError({
        statusCode: 403,
        statusMessage: 'Forbidden - Insufficient permissions to delete this variable'
      })
    }

    const data = await deleteVariable(validationId.data)
    if (!data) {
      throw createError({
        statusCode: 404,
