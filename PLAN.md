...
## Data Model (Supabase `tb` Schema & `auth.users`):

The application manages data defined by the following structure:

- `tb.data_category`
- `tb.data_source`
- `tb.organisation_element`
- `tb.variable`
- `tb.profile`
- `tb.role`
- `tb.indicator`
- `tb.data_entry`
- `auth.users`: Standard Supabase Auth table (relevant for FKs from `tb.organisation_element`, `tb.profile`, `tb.data_entry`)

### Detailed Schema per Table (`tb` schema)

Below are the detailed columns and constraints for each table in the `tb` schema:
...
- **`tb.data_source`**:
  ...
  - `designation`: Text
  - `type`: Text
  - `parent_id`: Integer (FK -> `tb.data_source.id` for hierarchy, NULLABLE)

- **`tb.organization_element`**:
...
- **`tb.variable`**:
  ...
  - `data_source_id`: Integer (FK -> `tb.data_source.id`, NOT NULL)
  - `category_code`: Text (FK -> `tb.data_category.code`, NOT NULL)
  - `frequency`: Text
  - `level`: Text
...
- **`tb.profile`**:
  ...
  - `organization_element_code`: Text (FK -> `tb.organization_element.code`, NULLABLE)
...
- **`tb.data_entry`**:
  - `id`: Integer (PK, SERIAL)
  - `created_at`: Timestamp with Time Zone (DEFAULT now())
  - `variable_code`: Text (FK -> `tb.variable.code`, NOT NULL, ON DELETE CASCADE)
  - `value`: Float4 (Floating point number)
  - `valid`: Boolean
  - `year`: Integer
  - `updated_at`: Timestamp with Time Zone (DEFAULT now())
  - `category_code`: Text (FK -> `tb.data_category.code`, NOT NULL)
  - `organization_element_code`: Text (FK -> `tb.organization_element.code`, NOT NULL)
...
- **Authorization (RBAC):** Implement Role-Based Access Control using `tb.role` and `tb.profile`. Secure API endpoints and UI elements based on user roles (based on `tb.profile.roles` array).
- **CRUD Operations:** Generate full Create, Read, Update, Delete interfaces and backend logic for _all_ tables in the `tb` schema, secured by RBAC.
...
- **Fulltext Search:** Implement a header search bar for searching across relevant fields (`code`, `designation`, `acronym`, etc. - derived from the detailed schema) in multiple `tb` tables, leveraging Supabase search capabilities.
...
- **User Dropdown Menu:** Links/actions for Profile (view/edit `tb.profile`), Preferences, Logout.
- **Sidebar Navigation:** Menu providing access to CRUD interfaces for `tb` tables and Dashboard.
...
- Configure Prisma schema based on the `tb` and `auth.users` tables, reflecting the detailed structure provided.
...
2.  **Step 1.2: Supabase & Prisma Schema Definition**
    ...
    - **Tasks:**
      ...
      - Configure Prisma schema (`prisma/schema.prisma`) based _precisely_ on the provided detailed `tb` schema and `auth.users` table structure, including relationships (FKs), scalar types, unique constraints, defaults, and `onDelete` actions (like `CASCADE`).
      ...
      - Draft a conceptual plan for Supabase Row Level Security (RLS) policies for each `tb` table, outlining how they will align with the RBAC roles defined later. Note tables where RLS will be critical (`data_entry`, `profile`, `organization_element`) and how RLS will interact with the detailed schema constraints.
...
4.  **Step 2.2: Role-Based Access Control (RBAC) Core**
    - **Objective:** Implement the core logic for checking user roles based on `tb.profile.roles`.
    - **Tasks:**
      - Enhance the auth Pinia store or create a new `profile.ts` store/composable to fetch and store the authenticated user's `tb.profile` including the `roles` array, leveraging the specific fields defined in the detailed schema (`roles`, `active`, `organisation_element_code`).
...
5.  **Step 3.1: API Layer Foundation (One CRUD Example)**
    ...
    - **Tasks:**
      ...
      - Implement basic CRUD routes (`GET`, `POST`, `PUT`, `DELETE`) for _one_ simple table (e.g., `tb.data_category`) in `server/api/data-categories/[id].ts` and `server/api/data-categories/index.ts`. Ensure these routes handle inputs/outputs based on the detailed `data_category` schema (`code`, `designation`).
...
6.  **Step 3.2: Main UI Layout & Navigation**
    ...
    - **Tasks:**
      ...
      - Implement the user avatar/dropdown component (`components/layout/UserDropdown.vue`) with Logout functionality hooked up to the auth store. Add placeholder links for Profile and Preferences. The profile link should navigate to a view/edit page for the user's `tb.profile` based on the detailed schema.
      - Implement the sidebar component (`components/layout/AppSidebar.vue`) using Tailwind and Shadcn-vue. Make it collapsible. Add placeholder navigation links for each `tb` table based on the table names from the detailed schema.
    ...
    - **Output:** `default.vue` layout, `AppHeader.vue`, `UserDropdown.vue` (with Profile link reflecting `tb.profile`), `AppSidebar.vue` (with links for all `tb` tables), `AppBreadcrumbs.vue` components, basic responsive layout structure, functional dark mode toggle.
...
8.  **Step 4.1 - 4.X: Generate CRUD for Each `tb` Table**
    - **Objective:** For _each_ table in the `tb` schema, generate the full CRUD interface, leveraging the detailed schema information for forms and tables.
    - **Tasks (Repeat for each table based on its detailed schema):**
      ...
      - **RBAC Integration:** Use the `useRoles` composable to conditionally display/enable/disable UI elements or actions (e.g., hide 'Create New' button if user lacks permission). Ensure backend checks are the primary security layer, checking against roles derived from the detailed `tb.profile` schema.
...
9.  **Step 5.1: Fulltext Search Implementation**
    ...
    - **Tasks:**
      - Identify _searchable fields_ across relevant `tb` tables based on the detailed schema (`code`, `designation`, `acronym`, `defintion`, `goal`, `formula`, `interpretation`, `example`).
